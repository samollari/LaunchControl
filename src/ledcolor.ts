...
export class RGBLEDColorDefinition implements SysExComponent {
    ...
    public constructor(ledIndex: number, red: number, green: number, blue: number) {
        assertIsLEDIndex(ledIndex);
...
export function assertIsStandardColor(...args: Parameters<typeof isStandardColor>): void {
    if (!isStandardColor(...args)) {
        throw RangeError('LED Color must be an integer between 0 and 127 (inclusive)');
    }
}
...
export function assertIsLEDIndex(...args: Parameters<typeof isLEDIndex>): void {
    if (!isLEDIndex(...args)) {
        throw RangeError('LED Index must be an integer between 0x01 and 0x63 (inclusive)');
    }
}
